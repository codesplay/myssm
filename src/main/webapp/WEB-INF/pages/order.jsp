		...
		<div class="order_wrapper">
			...
			<div class="order_info clearfix">
				<div class="order_tab clearfix">
					<ul>
						<li><a style="border-bottom: red 2px solid" href="#">全部订单</a></li>
						<li><a href="#">待付款</a></li>
						<li><a href="#">待收货</a></li>
						<li><a href="mycomments?id=${sessionScope.user.id}">待评价</a></li>
					</ul>
				...
				<div class="order_main">
					<table class="order_table">
						...
						<c:forEach items="${OrderList}" var="bean" varStatus="loop">
							<tbody>
								<tr class="order_row"></tr>
								<tr class="order_tr">
									<td colspan="5"><span class="gap"></span> <span
										class="order_time">${bean.orderList[0].ordertime }</span> <span
										class="order_num"> 订单号：<a href="#">${bean.orderList[0].ordernum }</a>
									</span></td>
								</tr>
								
								<c:forEach items="${bean.orderList}" var="orderbean" varStatus="prolist">
								<tr class="tr">
									<td class="order_width">
										<div class="order_goods">
											<div class="order_pic">
												<img style="width: 70px; heigth: 70px"
													src="images/${bean.productList[prolist.index].proimg}" />
											</div>
											<div class="order_msg">
												<a class="order_product" href="toproduct/${bean.productList[prolist.index].id}">${bean.productList[prolist.index].prodesc }</a>
											</div>
										</div>
										<div class="order_goodsnum">x${orderbean.buynum }</div>
									</td>
									<td rowspan="1" style="width: 110px"><div
											class="order_user">
											<span class="txt">${orderbean.orderuser }</span><b></b>
										</div></td>
									<td rowspan="1"><div class="order_priceall">
											<span class="order_pricenum">总额 ￥${orderbean.buymoney }</span>
										</div></td>
									<td rowspan="1"><div class="order_stat">
											
											<c:if test="${orderbean.status<0 }">已取消</c:if>
											<c:if test="${orderbean.status==0 }"><span class="order_statmsg">未付款</span></c:if>
											<c:if test="${orderbean.status==1 }"><span class="order_statmsg">已付款</span></c:if>
											<c:if test="${orderbean.status==2 }"><span class="order_statmsg">已评价</span></c:if>
										</div></td>
									<td rowspan="${bean.order_nums }"><div class="order_op">
											<c:if test="${prolist.index == 0}">
											<c:if test="${orderbean.status == 0 }">
												<div>
													<form action="alipay/pay" method="post">
														<input type="hidden" name="WIDout_trade_no" id="WIDout_trade_no" value="${orderbean.ordernum }">
														<a class="order_buynow" href="javascript:void(0);"><input type="submit" style="cursor: pointer;background:#fff;margin: 0;padding: 0;border: 1px solid transparent;outline: none" value="立即购买"/></a>
													</form>
												</div>
											</c:if>
								...
								<tr class="tr">
									<td class="order_width">
										<div class="order_goods">
											...
											<div class="order_msg">
												<a class="order_product" href="#">${products[loop.count-1].prodesc }</a>
											</div>
										</div>
									...
									<td rowspan="1" style="width: 110px"><div
											class="order_user">
											<span class="txt">${username[loop.count-1] }</span><b></b>
										</div></td>
									<td rowspan="1"><div class="order_priceall">
									...
									<td rowspan="1"><div class="order_op">
											<c:if test="${bean.status == 0 }">
												<form action="alipay/pay" method="post">
													<input type="hidden" name="WIDout_trade_no" id="WIDout_trade_no" value="${bean.ordernum }"> "> 
													<input type="hidden" name="WIDtotal_amount" id="WIDtotal_amount" value="${bean.buymoney }"> 
													<input type="hidden" name="WIDbody" id="WIDbody" value=" ">
												</form>
												<div>
													<a class="order_buynow" href="javascript:void(0);"
														onclick="payorder()">立即购买</a>
												</div>
											</c:if>
